 ...
 * 
 * QoS4SendDaemon.java at 2021-7-1 15:08:17, code by Jack Jiang.
 */
...
public class QoS4SendDaemon {
    ...
    private void init() {
        ...
        runnable = () -> {
            if (!_excuting) {
                final ArrayList<Message> lostMessages = new ArrayList<>();
                MBThreadPoolExecutor.runInBackground(() -> {
                    final ArrayList<Message> ret = doRetryCheck(lostMessages);
    ...
    private ArrayList<Message> doRetryCheck(ArrayList<Message> lostMessages) {
        _excuting = true;
        try {
            ...
            for (String key : sentMessages.keySet()) {
                final Message msg = sentMessages.get(key);
                if (msg != null && msg.isQoS()) {
                    if (msg.getRetryCount() >= QOS_TRY_COUNT) {
                        if (ClientCoreSDK.DEBUG)
                            Log.d(TAG, "【IMCORE-TCP】【QoS】指纹为" + msg.getFp()
                                    + "的消息包重传次数已达" + msg.getRetryCount() + "(最多" + QOS_TRY_COUNT + "次)上限，将判定为丢包！");

                        lostMessages.add((Message) msg.clone());
                        remove(msg.getFp());
                    } else {
                        ...
                        //### 2015103 Bug Fix END
                            ...
                            new LocalDataSender.SendCommonDataAsync(msg) {
                                @Override
                                protected void onPostExecute(Integer code) {
                                    msg.increaseRetryCount();
                                    if (code == 0) {
                                        if (ClientCoreSDK.DEBUG)
                                            Log.d(TAG, "【IMCORE-TCP】【QoS】指纹为" + msg.getFp() + "的消息包已成功进行重传，此次之后重传次数已达" + msg.getRetryCount() + "(最多" + QOS_TRY_COUNT + "次).");
                                    } else {
                                        Log.w(TAG, "【IMCORE-TCP】【QoS】指纹为" + msg.getFp() + "的消息包重传失败，它的重传次数之前已累计为" + msg.getRetryCount() + "(最多" + QOS_TRY_COUNT + "次).");
                                    }
                                }
                            }.execute();
                        }
    ...
    private void onRetryCheck(ArrayList<Message> al) {
        // for DEBUG
    ...
    public void put(Message msg) {
        ...
        if (msg.getFp() == null) {
            Log.w(TAG, "Invalid arg msg.getFp() == null.");
            return;
        }

        if (!msg.isQoS()) {
            Log.w(TAG, "This msg is not QoS pkg, ignore it!");
            return;
        }

        if (sentMessages.get(msg.getFp()) != null)
            Log.w(TAG, "【IMCORE-TCP】【QoS】指纹为" + msg.getFp() + "的消息已经放入了发送质量保证队列，该消息为何会重复？（生成的指纹码重复？还是重复put？）");

        sentMessages.put(msg.getFp(), msg);
        sendMessagesTimestamp.put(msg.getFp(), System.currentTimeMillis());
    }

    public void remove(final String fingerPrint) {
        sendMessagesTimestamp.remove(fingerPrint);
        Message removedObj = sentMessages.remove(fingerPrint);
        Log.w(TAG, "【IMCORE-TCP】【QoS】指纹为" + fingerPrint + "的消息已成功从发送质量保证队列中移除(可能是收到接收方的应答也可能是达到了重传的次数上限)，重试次数="
                + (removedObj != null ? (removedObj).getRetryCount() : "none呵呵."));
    }
