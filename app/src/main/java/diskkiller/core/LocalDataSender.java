 ...
 * 
 * LocalDataSender.java at 2021-7-1 15:08:17, code by Jack Jiang.
 */
...
import diskkiller.PLoginInfo;
import diskkiller.msgBean.Message;
import diskkiller.utils.MBAsyncTask;
import diskkiller.utils.MBObserver;
import diskkiller.utils.TCPUtils;
...
public class LocalDataSender {
    ...
    int sendLoginImpl(PLoginInfo loginInfo) {
        ...
        String senLoginMsg = new Gson().toJson(loginMsg);
        int code = send(senLoginMsg);
        if (code == 0) {
    ...
    public int sendLoginout() {
        ...
        if (ClientCoreSDK.getInstance().isLoginHasInit()) {
            ...
            String senLogoutMsg = new Gson().toJson(logoutMsg);
            code = send(senLogoutMsg);
            if (code == 0) {
                // do nothing
            }
        }
    ...
    int sendKeepAlive() {
        ...
        String senHeartMsg = new Gson().toJson(heartMsg);
        return send(senHeartMsg);
    }
    ...
    public int sendCommonData(String dataContentWidthStr, String to_user_id, boolean QoS, String fingerPrint, int typeu) {
        return sendCommonData(new Message());
    }

    public int sendCommonData(Message msg) {
        if (msg != null) {

            String senMsg = new Gson().toJson(msg);

            int code = send(senMsg);
            if (code == 0) {
                if (msg.isQoS() && !QoS4SendDaemon.getInstance().exist(msg.getFp()))
                    QoS4SendDaemon.getInstance().put(msg);
            }
            return code;
        } else
    ...
    private int send(String fullStringData) {

        int codeForCheck = this.checkBeforeSend();
        if (codeForCheck != ErrorCode.COMMON_CODE_OK)
            return codeForCheck;

        WebSocket webSocket = LocalSocketProvider.getInstance().getLocalSocket();
        if (webSocket != null) {// && [ClientCoreSDK sharedInstance].connectedToServer)
            return TCPUtils.send(webSocket, fullStringData) ? ErrorCode.COMMON_CODE_OK : ErrorCode.COMMON_DATA_SEND_FAILD;
        } else {
            Log.d(TAG, "【IMCORE-TCP】scocket未连接，无法发送，本条将被忽略（fullStringData=" + fullStringData + "）!");
    ...
    public static abstract class SendCommonDataAsync extends MBAsyncTask {
        ...
        public SendCommonDataAsync(Message m) {
            if (m == null) {
        ...
        protected Integer doInBackground(Object... params) {
            if (m != null)
                return LocalDataSender.getInstance().sendCommonData(m);//dataContentWidthStr, to_user_id);
            return -1;
        }
