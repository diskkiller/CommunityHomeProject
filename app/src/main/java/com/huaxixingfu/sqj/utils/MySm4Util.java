...
public class MySm4Util {
    ...
    void MySm4Crypt(byte[] Input, byte[] Output, int[] rk) {
    ...
    private void MySm4KeyExt(byte[] Key, int[] rk, int CryptFlag) {
        int r, mid;
        int[] x = new int[4];
        int[] tmp = new int[4];
        for (int i = 0; i < 4; i++) {
     ...
     * SM4的加密、解密
     *
     * @param in        如果是加密则代表明文，如果是解密则代表密文
     * @param inLen     字节长度
     * @param key       密钥
     * @param out       运算结果，如果是加密返回的是密文，如果是解密则返回的是明文
     * @param CryptFlag 1加密 0解密
     * @return
     */
    public int EnOrDecrypt(byte[] in, int inLen, byte[] key, byte[] out, int CryptFlag) {
        int point = 0;
        int[] round_key = new int[ROUND];
        //int[] round_key={0};
        MySm4KeyExt(key, round_key, CryptFlag);
        byte[] input = new byte[16];
        byte[] output = new byte[16];
        while (inLen >= BLOCK) {
            input = Arrays.copyOfRange(in, point, point + 16);
            //output=Arrays.copyOfRange(out, point, point+16);
            MySm4Crypt(input, output, round_key);
