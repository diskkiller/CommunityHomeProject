...
public class IdCardUtil {
    ...
    public String getSex() {
        ...
        int p = AppValidationMgr.parseIntStringVidialty(getOrder());
        if (p % 2 == 1) {
            return "男";
        } else {
            return "女";
    ...
    private boolean isDateCorrect() {
        ...
        int month;
        if (this.is15()) {
            month = AppValidationMgr.parseIntStringVidialty(this.idCardNum.substring(8, 10));
        } else {
            month = AppValidationMgr.parseIntStringVidialty(this.idCardNum.substring(10, 12));
        }

        int day;
        if (this.is15()) {
            day = AppValidationMgr.parseIntStringVidialty(this.idCardNum.substring(10, 12));
        } else {
            day = AppValidationMgr.parseIntStringVidialty(this.idCardNum.substring(12, 14));
        }
    ...
    private String getCheckBit() {
        ...
        for (int i = 0; i < 17; i++) {
            ...
            if(StringUtils.isNotEmpty(ch)){
                sum = sum + AppValidationMgr.parseIntStringVidialty(ch) * wi[i];
            }
        }
    ...
    private boolean isLeapyear() {
        ...
        int year = AppValidationMgr.parseIntStringVidialty(temp);
