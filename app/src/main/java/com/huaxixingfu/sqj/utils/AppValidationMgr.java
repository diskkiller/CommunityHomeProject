...
public class AppValidationMgr {
	...
	// 包含浮点型
	private final static Pattern number_float_pattern = Pattern.compile("^[0-9\\.]*$");
...
//^(?=.*\\d)(?=.*[a-z])(?=.*[A-Z]).{8,10}$
	...
	public static boolean isNumberFloat(String str) {
		return number_float_pattern.matcher(str).matches();
	}
	 ...
	 * @return boolean
	 */
	public static boolean isColorsText(String username) {
		return color_patter.matcher(username).matches();
	}
...
//	}
    ...
    public static int parseIntStringVidialty(String target){
        if(AppValidationMgr.isNumber(target)){
